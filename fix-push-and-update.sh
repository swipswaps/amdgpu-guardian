#!/bin/bash
# fix-push-and-update.sh – Resolve push conflict, push changes, and fix updater script.
# No set -e, no sed, no 2>/dev/null. Keeps terminal open.

echo "═══════════════════════════════════════════════════════════"
echo "  Fixing Push Conflict & Updating Helper Script"
echo "═══════════════════════════════════════════════════════════"

# 1. Pull remote changes with rebase
echo ""
echo "[1] Pulling remote changes (rebase)..."
git pull origin master --rebase
if [ $? -ne 0 ]; then
    echo "❌ Pull failed. Please resolve conflicts manually, then run:"
    echo "   git push origin master"
    echo "   And then: ./update-db-path.sh (to update the helper script)"
    exit 1
fi
echo "✅ Pull successful."

# 2. Push the combined changes
echo ""
echo "[2] Pushing to origin/master..."
...
# 3. Fix update-db-path.sh to correctly report push status
echo ""
echo "[3] Fixing update-db-path.sh (add push exit code check)..."

# Backup the existing updater script
if [ -f update-db-path.sh ]; then
    cp update-db-path.sh update-db-path.sh.bak
    echo "  Backup saved as update-db-path.sh.bak"
fi

# Write a corrected version of update-db-path.sh that checks git push exit code
cat > update-db-path.sh <<'INNEREOF'
#!/bin/bash
...
chmod +x update-db-path.sh
echo "✅ update-db-path.sh has been fixed and now checks push exit codes."

echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  All fixes applied. Your repository is now up-to-date."
echo "  The XDG database patch is applied and the helper script is fixed."
